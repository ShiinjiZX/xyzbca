...
import { Terminal, Play, Square, Trash2, Upload, FolderOpen, FileText, Folder, Archive, Download, Github, Save } from 'lucide-react';
...
export default function NodeTerminal() {
  ...
  const loadFromStorage = async () => {
    try {
      const storedFiles = await window.storage.get('nodejs-files');
      if (storedFiles) {
        const filesData = JSON.parse(storedFiles.value);
        setFiles(filesData);
  ...
  const saveToStorage = async (filesData) => {
    try {
      await window.storage.set('nodejs-files', JSON.stringify(filesData));
      addOutput('Files saved to storage', 'success');
    } catch (err) {
...
Run: npm start`
    ...
    await saveToStorage(exampleFiles);
